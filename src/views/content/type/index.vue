...
export default {
  ...
  methods: {
    ...
    handleCreate() {
      this.list.push({ typeId: null, name: "", edit: true });
    },
    /** *
     * 删除数据
     */
    deleted(row) {
      ...
      this.$confirm('是否确认删除用户编号为"' + typeId + '"的数据项?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      })
        .then(function() {
          return deleteTypeById(typeId);
        })
        .then(() => {
          this.getList();
     ...
     */
    confirmEdit(row) {
      if (row.typeId && row.name) {
        updateType(row.typeId, row.name).then(response => {
          row.edit = false;
