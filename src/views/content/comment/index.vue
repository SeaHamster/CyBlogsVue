  ...
  <div class="app-container">
    ...
    >
      <el-table-column
        ...
        align="center"
      ></el-table-column>
      <!-- <el-table-column prop="commentId" label="ID" width="60"></el-table-column> -->
...
export default {
  ...
  created() {
    this.getBlogDicList();
    this.getComments(this.queryParams.blogId);
  },
  methods: {
    /** *
     * 获取博客字典列表
    ...
    getBlogDicList() {
      hasCommentDic().then(response => {
        ...
        this.queryParams.blogId = response[0].blogId;
        this.getComments(this.queryParams.blogId);
      });
    },
    /** *
    ...
    getComments(blogId) {
      this.loading = true;
      getCommentWithChildById(blogId).then(response => {
    ...
    handleModifyStatus(row, status) {
      setDeleted(row.commentId, status).then(response => {
        this.$message({
          message: "操作Success",
          type: "success"
        });
